...
description: "Use when working on SportMania Backend architecture and implementation: .NET 10 Web API, controllers/services/repositories, EF Core with PostgreSQL, JWT auth, ToyyibPay flow, Discord bot integration, and production-safe refactoring with self-learning memory."
...
## Repository Context (SportMania Backend)

Use these repository-specific defaults unless the user explicitly requests otherwise:

- Project type: ASP.NET Core Web API on .NET 10
- API style: Controller-based APIs are the established pattern in this codebase
- Data access: EF Core with Npgsql (PostgreSQL)
- Architecture layers:
  - `Backend/Controllers` for HTTP endpoints and response mapping
  - `Backend/Services` for business logic and orchestration
  - `Backend/Repository` for EF Core query and persistence logic
  - `Backend/Data/ApplicationDbContext.cs` for model configuration and DB context
- Integrations:
  - ToyyibPay payment initiation and callback handling
  - Optional Discord bot + role mapping, gated by configuration
- Security baseline:
  - JWT bearer authentication configured from `Auth` section
  - Authorization enforced at endpoint level

## Repository-Specific Rules

- Keep controllers thin: delegate business logic to services.
- Keep services stateless and async-first; include `CancellationToken cancellationToken = default` in async contracts.
- Keep repositories focused per entity and use async EF Core methods.
- Never bypass repositories by injecting `ApplicationDbContext` into controllers.
- Use DTOs for request/response contracts; do not expose EF entities directly in API responses.
- Preserve current dependency injection patterns in `Backend/Program.cs`.
- For read-only EF queries, prefer `AsNoTracking()`.
- In services and repositories, use `ConfigureAwait(false)` on awaits.

## Build And Validation Defaults

When validating implementation in this repository, use:

1. `dotnet restore SportMania.sln`
2. `dotnet build SportMania.sln`
3. `dotnet test SportMania.sln` (if tests are present)

For backend local run:
